# -*- coding: utf-8 -*-
"""تحليل الصور في نقاط - النسخة 3.7 (إصلاح الصفحة البيضاء)"""

# ========== استيراد streamlit أولاً ==========
import streamlit as st

# ========== فحص المكتبات قبل أي شيء ==========
missing_libs = []
...
# إذا كانت هناك مكتبات مفقودة، أوقف التطبيق وأظهر رسالة
if missing_libs:
    st.error(f"❌ مكتبات مفقودة: {', '.join(missing_libs)}")
    st.code("\n".join(missing_libs), language="text")
    st.info("📝 أضف المكتبات المفقودة في ملف requirements.txt ثم أعد النشر")
    st.stop()
...
def generate_executive_summary(results, extracted_text=""):
    ...
    def short(txt, n=120):
        return (txt[:n] + "...") if len(str(txt)) > n else txt
...
def analyze_post_smart(text, mentions):
    ...
    results["المدعو"]         = first_match([r'المدعو\s+([@\w\s]+)', r'الشيخ\s+([@\w\s]+)'], text)
...
"""
        ...
        for model_name in models_to_try:
            try:
                ...
                if any(x in err_str for x in ["QUOTA_EXCEEDED", "429", "quota",
                    ...
                    last_error = f"❌ {model_name}: {err_str[:100]}"
                    continue

        return None, (
            f"❌ فشلت جميع نماذج Gemini.\n\n"
            f"آخر خطأ: {last_error}\n\n"
            f"💡 الحلول:\n"
            f"• انتظر 60 ثانية وأعد المحاولة\n"
            f"• استخدم مفتاح API جديد\n"
...
with st.sidebar:
    ...
    if st.session_state.used_model:
        st.markdown(
            f'<span class="model-badge">🤖 {st.session_state.used_model}</span>',
            unsafe_allow_html=True
        )

    if st.session_state.analysis_done:
        if st.button("🗑️ مسح النتائج", use_container_width=True):
            for k in ["analysis_done", "results", "extracted_text",
                      "analysis_method", "used_model"]:
                st.session_state[k] = (
                    False if k == "analysis_done"
                    else None if k == "results"
                    else ""
                )
            st.rerun()
...
if uploaded_file:
    ...
    with col1:
        ...
        st.caption(
            f"📐 {image.size[0]}×{image.size[1]} بكسل | "
    ...
    with col2:
        ...
        if st.button("🔍 تحليل الصورة الآن", use_container_width=True):
            ...
            with st.spinner("⏳ جاري تحليل الصورة..."):
                ...
                if use_gemini:
                    results, error, used_model = analyze_with_gemini(
...
if st.session_state.analysis_done and st.session_state.results:
    ...
    filled = sum(1 for v in res.values()
                 if v not in ["غير مُحدد", "غير محدد", "", None])
    total  = len(res)
    pct    = int(filled / total * 100)

    model_info = (
        f"| النموذج: <strong>{st.session_state.used_model}</strong>"
        if st.session_state.used_model else ""
    )
    st.markdown(f"""
    <div class="success-banner">
        <h3>✅ تم التحليل بنجاح!</h3>
        <p>طريقة التحليل: <strong>{st.session_state.analysis_method}</strong> {model_info}</p>
    ...
    if st.session_state.extracted_text:
        ...
        if cats:
            ...
            for cat, words in cats.items():
                css = (
                    "sarcastic" if "تهكم" in cat
                    else "political" if "سياسية" in cat
                    else "general"
                )
                for w in words[:3]:
...
with st.expander("📖 دليل الاستخدام"):
    st.markdown("""
### خطوات التحليل:
1️⃣ **اختر طريقة التحليل** من الشريط الجانبي
...
- ✅ فحص المكتبات عند التشغيل
- ✅ Fallback تلقائي عند تجاوز الحصة
- ✅ روابط X قابلة للنقر
...
st.markdown("---")
st.caption(
    "© تحليل الصور في نقاط - الإصدار 3.7 | "
