# ============================================================
#  محلل حسابات X  – v9.9
#  إضافة: تضمين الصور في تصدير Word و PowerPoint
# ============================================================
...
def b64_to_bytes(b64: str) -> bytes:
    """تحويل base64 إلى bytes للإدراج في Office"""
    try:
        ...
        try:
            ...
            for i, lel in enumerate(labels):
                ...
                if 'follow' in lbl and 'ing' not in lbl:
                    ...
                    following = val
                elif 'tweet' in lbl or 'post' in lbl:
                    tweets = val
            if followers == 0:
...
def export_to_word(
    ...
    images_b64: list = None,        # ← قائمة صور base64
) -> bytes:
    """تصدير التقرير إلى Word مع تضمين الصور"""
    try:
        ...
        def add_heading(text, level=1):
            colors = {1: (31, 111, 235), 2: (88, 166, 255), 3: (139, 148, 158)}
            sizes  = {1: 18, 2: 14, 3: 12}
            add_para(text, bold=True, size=sizes.get(level, 12), color=colors.get(level))

        def add_divider():
            add_para('─' * 55, size=9, color=(48, 54, 61))

        def insert_image_b64(b64: str, width_inches: float = 3.0, caption: str = ''):
            """إدراج صورة من base64 في المستند"""
            img_bytes = b64_to_bytes(b64)
            if not img_bytes:
                return
            try:
                # تحقق من الصورة وتحويلها
        ...
        if imgs:
            if export_type == 'account':
                ...
                insert_image_b64(imgs[0], width_inches=2.5, caption="صورة الحساب")
                for extra in imgs[1:]:
                    insert_image_b64(extra, width_inches=3.5, caption="صورة إضافية")
            else:
                add_heading("صور التغريدة", level=2)
                for idx, img_b64 in enumerate(imgs):
                    lbl = f"صورة {idx + 1}"
                    insert_image_b64(img_b64, width_inches=4.0, caption=lbl)
            add_divider()
            doc.add_paragraph()

        # ── بيانات الحساب / التغريدة ────────────────────────
        if export_type == 'account':
        ...
        add_divider()
        add_para(
            "أُنتج بواسطة محلل حسابات X الاستخباراتي | Gemini AI | v9.9",
...
def export_to_pptx(
    ...
    """تصدير التقرير إلى PowerPoint مع تضمين الصور"""
    try:
        ...
        def add_tb(sl, text, l, t, w, h,
                   size=16, bold=False, color=None, align=PP_ALIGN.RIGHT, italic=False):
        ...
        def add_rect(sl, l, t, w, h, color, alpha=None):
        ...
        def add_top_bar(sl):
            add_rect(sl, 0, 0,    13.33, 0.07, BLUE)

        def add_bottom_bar(sl):
            add_rect(sl, 0, 7.43, 13.33, 0.07, BLUE)

        def insert_image_pptx(sl, b64: str, l: float, t: float,
                               max_w: float, max_h: float) -> bool:
            """إدراج صورة في شريحة مع الحفاظ على النسبة"""
            img_bytes = b64_to_bytes(b64)
            if not img_bytes:
                return False
            try:
                ...
                # احسب الأبعاد مع الحفاظ على نسبة الجانب
                orig_w, orig_h = img.size
                ratio = orig_w / orig_h
                if ratio >= 1:          # أفقية أو مربعة
        ...
        add_tb(sl1, "Gemini AI  |  v9.9",
               1.7, 4.7, 10, 0.5, size=12, color=ACCENT, align=PP_ALIGN.CENTER)

        # ════════════════════════════════════════════════════
        # الشريحة 2: الصور (إن وجدت)
        # ════════════════════════════════════════════════════
        imgs = [i for i in (images_b64 or []) if i]
        if imgs:
            sl_img = new_slide()
            add_top_bar(sl_img)
            add_bottom_bar(sl_img)

            if export_type == 'account':
                add_tb(sl_img, "📸 صورة الملف الشخصي",
                       0.5, 0.15, 12, 0.7, size=22, bold=True, color=ACCENT)
                add_rect(sl_img, 0.5, 0.88, 12, 0.04, BLUE)

                # صورة البروفايل (مربعة كبيرة في المنتصف)
                insert_image_pptx(sl_img, imgs[0],
                                  l=4.2, t=1.1, max_w=5.0, max_h=5.0)

                # صور إضافية (إن وجدت) على الجانب
                extra_y = 1.1
                for extra_b64 in imgs[1:3]:
                    insert_image_pptx(sl_img, extra_b64,
                                      l=9.7, t=extra_y, max_w=3.0, max_h=2.2)
                    extra_y += 2.4

                add_tb(sl_img, f"@{data.get('screen_name', '')}  –  {data.get('name', '')}",
                       0.5, 6.6, 12, 0.6, size=14, color=GRAY, align=PP_ALIGN.CENTER)
                ...
                for idx, pos in enumerate(positions[count]):
                    insert_image_pptx(sl_img, imgs[idx],
                                      l=pos[0], t=pos[1], max_w=pos[2], max_h=pos[3])
                    add_tb(sl_img, f"صورة {idx + 1}",
                           pos[0], pos[1] + pos[3] + 0.05, pos[2], 0.35,
                           size=12, color=GRAY, align=PP_ALIGN.CENTER)

        # ════════════════════════════════════════════════════
        # الشريحة 3: بيانات الحساب / التغريدة
        # ════════════════════════════════════════════════════
        sl2 = new_slide()
        add_top_bar(sl2)
        add_bottom_bar(sl2)
        ...
        if export_type == 'account':
            add_tb(sl2, "👤 بيانات الحساب",
                   0.5, 0.15, 12, 0.7, size=22, bold=True, color=ACCENT)
            add_rect(sl2, 0.5, 0.88, 12, 0.04, BLUE)
            ...
            # عمودان
            left_fields  = fields[:4]
            right_fields = fields[4:]

            y = 1.05
            for lbl, val in left_fields:
                if val:
                    add_tb(sl2, f"{lbl}:", 6.6, y, 3.0, 0.45,
                           size=13, bold=True, color=ACCENT)
                    add_tb(sl2, val, 0.5, y, 6.0, 0.45,
                           size=13, color=WHITE)
                    y += 0.52
            ...
            for lbl, val in right_fields:
                if val:
                    add_tb(sl2, f"{lbl}:", 6.6, y2, 3.0, 0.45,
                           size=13, bold=True, color=ACCENT)
                    add_tb(sl2, val, 0.5, y2, 6.0, 0.45,
                           size=13, color=WHITE)
                    y2 += 0.52
            ...
            bio = data.get('bio', '')
            if bio:
                add_tb(sl2, "الوصف:", 12.0, 3.9, 1.2, 0.4, size=12, bold=True, color=ACCENT)
                bio_short = bio[:180] + ('…' if len(bio) > 180 else '')
                add_tb(sl2, bio_short, 0.5, 3.9, 11.5, 1.2, size=12, color=GRAY)
            ...
            for lbl, val, col in stats_data:
                add_rect(sl2, sx, 5.35, 3.8, 1.5, CARD)
                add_tb(sl2, val, sx, 5.5, 3.8, 0.8,
                       size=28, bold=True, color=col, align=PP_ALIGN.CENTER)
                add_tb(sl2, lbl, sx, 6.2, 3.8, 0.45,
            ...
            for lbl, val in fields:
                if val:
                    add_tb(sl2, f"{lbl}:", 12.0, y, 1.2, 0.4,
                           size=13, bold=True, color=ACCENT)
                    add_tb(sl2, val, 0.5, y, 11.4, 0.4, size=13, color=WHITE)
                    y += 0.5

            tweet_text = data.get('text', '')
            if tweet_text:
                add_rect(sl2, 0.5, y + 0.1, 12.33, 1.8, CARD)
                short = tweet_text[:220] + ('…' if len(tweet_text) > 220 else '')
                add_tb(sl2, short, 0.7, y + 0.2, 12.0, 1.6,
                       size=14, color=WHITE)
        ...
        if report_text:
            ...
            for cidx, chunk in enumerate(chunks):
                ...
                slide_t = "📊 تقرير التحليل الاستخباراتي"
                if len(chunks) > 1:
                    slide_t += f"  ({cidx + 1} / {len(chunks)})"
                add_tb(sl_r, slide_t, 0.5, 0.15, 12, 0.7,
                       size=20, bold=True, color=ACCENT)
                add_rect(sl_r, 0.5, 0.88, 12, 0.04, BLUE)
                ...
                for line in chunk:
                    ...
                    if is_h1 or is_h2:
                        add_rect(sl_r, 0.5, y - 0.04, 12, 0.52, CARD)
                        add_tb(sl_r, txt, 0.6, y, 11.8, 0.5,
        ...
        # ════════════════════════════════════════════════════
        # الشريحة الأخيرة: خاتمة
        # ════════════════════════════════════════════════════
        sl_end = new_slide()
        add_top_bar(sl_end)
        add_bottom_bar(sl_end)
        add_rect(sl_end, 2.0, 1.8, 9.33, 3.5, CARD)
        add_tb(sl_end, "✅ انتهى التقرير",
               2.0, 2.2, 9.33, 1.2, size=34, bold=True, color=ACCENT, align=PP_ALIGN.CENTER)
        add_rect(sl_end, 3.5, 3.4, 6.33, 0.05, BLUE)
        add_tb(sl_end, "محلل حسابات X الاستخباراتي",
               2.0, 3.5, 9.33, 0.6, size=16, color=WHITE, align=PP_ALIGN.CENTER)
        add_tb(sl_end, "Gemini AI  |  v9.9",
               2.0, 4.15, 9.33, 0.5, size=13, color=GRAY, align=PP_ALIGN.CENTER)
...
def render_export_buttons(
    ...
    report_text: str,
    export_type: str = 'account',
    images_b64: list = None,        # ← تمرير الصور
):
    ...
    with col1:
        docx_b = export_to_word(title, data, report_text, export_type, images_b64)
        if docx_b:
            st.download_button(
                label="📄 تحميل Word",
                data=docx_b,
    ...
    with col2:
        pptx_b = export_to_pptx(title, data, report_text, export_type, images_b64)
        if pptx_b:
            st.download_button(
...
def render_profile_card(data: dict, featured_b64: str = None):
    ...
    <div class="profile-card">
        ...
        <div class="profile-stats">
            <div class="stat-box"><div class="stat-value">{followers}</div><div class="stat-label">متابِع</div></div>
            <div class="stat-box"><div class="stat-value">{following}</div><div class="stat-label">يتابع</div></div>
            <div class="stat-box"><div class="stat-value">{tweets}</div><div class="stat-label">تغريدة</div></div>
        </div>
...
def account_tab(gemini_key: str, gemini_model_name: str):
    ...
    with c2:
        if st.button("🗑️ مسح", key='clear_account'):
            for k in ['account_data', 'account_report']:
    ...
    if 'account_data' in st.session_state:
        ...
        if 'error' in data and not data.get('name'):
            ...
            with st.expander("✏️ إدخال يدوي"):
                c1, c2 = st.columns(2)
                with c1:
                    mn  = st.text_input("الاسم", key='m_name')
                    mh  = st.text_input("المعرّف", key='m_handle')
                    mf  = st.text_input("المتابِعون", key='m_f')
                    mfg = st.text_input("يتابع", key='m_fg')
                with c2:
                    mt  = st.text_input("التغريدات", key='m_t')
                    ml  = st.text_input("الموقع", key='m_l')
                    mj  = st.text_input("تاريخ الانضمام", key='m_j')
                    mb  = st.text_area("الوصف", key='m_b', height=80)
                if st.button("💾 حفظ", key='save_manual'):
                    st.session_state['account_data'] = {
                        'name': mn, 'screen_name': mh.lstrip('@'),
                        'followers': mf, 'following': mfg, 'tweets': mt,
                        'location': ml, 'joined': mj, 'bio': mb,
                        'source': 'إدخال يدوي',
                    }
                    st.rerun()
        else:
            ...
            # ── Gemini ──
            st.markdown('<div class="section-title">🤖 تقرير استخباراتي بالذكاء الاصطناعي</div>',
                        unsafe_allow_html=True)
            if not gemini_key:
                ...
                if st.button("🚀 توليد التقرير", key='gen_account_report'):
                    ...
                    if model:
                        prompt = f"""أنت محلل استخباراتي متخصص في تحليل حسابات X (تويتر).
...
قدّم التقرير باللغة العربية بشكل احترافي."""

                        imgs_for_ai = []
                        if featured_b64:
                            imgs_for_ai.append(featured_b64)
                        elif data.get('profile_image_b64'):
                            imgs_for_ai.append(data['profile_image_b64'])

                        with st.spinner("🤖 جاري التحليل..."):
                            if imgs_for_ai:
                                report = gemini_with_images(model, prompt, imgs_for_ai)
                            else:
            ...
            if 'account_report' in st.session_state:
                ...
                export_imgs = []
                if featured_b64:
                    export_imgs.append(featured_b64)
...
def tweet_tab(gemini_key: str, gemini_model_name: str):
    ...
    with c2:
        if st.button("🗑️ مسح", key='clear_tweet'):
            for k in ['tweet_data', 'tweet_report', 'tweet_media_b64']:
                st.session_state.pop(k, None)
            st.rerun()
    ...
    if 'tweet_data' in st.session_state:
        ...
        if 'error' in td:
            ...
            st.markdown(f'<div class="error-box">❌ {err}</div>', unsafe_allow_html=True)
            with st.expander("✏️ إدخال يدوي للتغريدة"):
                manual_text = st.text_area("نص التغريدة", key='mt_text', height=120)
                mc1, mc2   = st.columns(2)
                with mc1:
                    ma = st.text_input("اسم المؤلف",   key='mt_author')
                    mh = st.text_input("معرّف المؤلف", key='mt_handle')
                    md = st.text_input("التاريخ",      key='mt_date')
                with mc2:
                    ml = st.text_input("الإعجابات", key='mt_likes')
                    mr = st.text_input("الإعادات",  key='mt_rt')
                    mp = st.text_input("الردود",    key='mt_rep')
            ...
            # دمج صور التغريدة مع الصور المرفوعة
            ...
            if all_images_b64 and gemini_key:
                if st.button("🔬 تحليل الصور", key='analyze_imgs'):
                    model = get_gemini_model(gemini_key, gemini_model_name)
                    if model:
...
- اكتب باللغة العربية"""
                        with st.spinner("🤖 جاري تحليل الصور..."):
                            ...
                            st.markdown(
                                f'<div class="report-section" dir="rtl">{safe_html_lines(result)}</div>',
                                unsafe_allow_html=True
                            )
...
قدّم التقرير باللغة العربية."""
            ...
            if 'tweet_report' in st.session_state:
                ...
                # ── جمع كل الصور للتصدير ──
                export_imgs = list(tweet_media_b64)          # صور التغريدة الأصلية
                for uf in (uploaded_imgs or []):              # الصور المرفوعة
                    try:
                        b64 = pil_to_base64(Image.open(uf))
                ...
                render_export_buttons(
                    ...
                    report_text=t_rpt,
                    export_type='tweet',
                    images_b64=export_imgs[:4],  # ← صور التغريدة + المرفوعات
                )
...
def render_sidebar() -> tuple:
    with st.sidebar:
        ...
        <div style="text-align:center;padding:16px 0;">
            ...
            <p style="color:#8b949e;font-size:.85rem;">v9.9 | Gemini 2.5</p>
        </div>
        """, unsafe_allow_html=True)
        st.markdown("---")
...
def main():
    ...
    <div style="text-align:center;padding:20px 0 10px;direction:rtl;">
        ...
        <p style="color:#8b949e;font-size:1rem;margin:8px 0 0;">
            تحليل متقدم للحسابات والتغريدات – Gemini 2.5 | Word | PowerPoint
        </p>
    </div>
    """, unsafe_allow_html=True)
