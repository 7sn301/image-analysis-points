...
IMAGE_ANALYSIS_POINTS = {
    "📍 تحليل الموقع الجغرافي": "حلّل هذه الصورة لتحديد الموقع الجغرافي المحتمل. ابحث عن: أسماء الأماكن والشوارع، اللافتات والعلامات، الطراز المعماري، الغطاء النباتي والمناخ، العلامات البارزة. قدّم تقريراً احترافياً.",
    "👤 تحليل الأشخاص والهويات": "حلّل الأشخاص في الصورة: الجنس والعمر التقديري، الملابس ودلالاتها، الإيماءات والتعبيرات، أي وثائق أو شارات مرئية، العلاقات بين الأشخاص.",
    "🚗 تحليل المركبات والمعدات": "حلّل المركبات والمعدات: الماركة والموديل، أرقام اللوحات، حالة المركبة، الاستخدام المحتمل.",
    "📄 تحليل الوثائق والنصوص": "استخرج وحلّل أي نصوص أو وثائق في الصورة.",
    "🔫 تحليل الأسلحة والتجهيزات": "حدّد وحلّل أي أسلحة أو تجهيزات عسكرية مرئية.",
    "🏗️ تحليل البنية التحتية": "حلّل البنية التحتية والمنشآت المرئية.",
    "📅 تحليل التوقيت والزمن": "حدّد الإطار الزمني من المؤشرات المرئية.",
    "⚠️ تحليل الأحداث والنشاطات": "حلّل الأحداث والنشاطات الجارية في الصورة.",
    "🔍 كشف التزوير والتلاعب": "افحص الصورة للكشف عن أي تزوير أو تلاعب.",
    "🎯 تحليل شامل ومتكامل": "قدّم تحليلاً شاملاً ومتكاملاً للصورة من جميع الجوانب الاستخباراتية.",
}
def safe_text(text):
    if text is None:
        return ""
...
def download_image_b64(url):
    ...
    for mirror in NITTER_MIRRORS:
        try:
            ...
            data["tweets_count"] = tweets_el.get_text(strip=True).replace(",","") if tweets_el else "0"
            data["following_count"] = following_el.get_text(strip=True).replace(",","") if following_el else "0"
            data["followers_count"] = followers_el.get_text(strip=True).replace(",","") if followers_el else "0"
...
def set_para_rtl_pptx(paragraph):
    try:
        ...
        try:
            ...
            pass
            def export_to_word(title, account_data=None, tweet_data=None, report_text="",
                   images_b64=None, image_analysis_text="", exec_summary=""):
    doc = Document()
    try:
        body = doc.element.body
        sectPr = body.get_or_add_sectPr()
        bidi_el = etree.SubElement(sectPr, docx_qn('w:bidi'))
    except:
        pass
    ...
    _set_rtl_para(cover_para)
    cover_run = cover_para.add_run("محلل حسابات X الاستخباراتي")
    _set_run_font(cover_run, font_size=26, bold=True, color=(0, 120, 212))
    doc.add_paragraph()
    title_para = doc.add_paragraph()
    _set_rtl_para(title_para)
    title_run = title_para.add_run(safe_text(title))
    _set_run_font(title_run, font_size=20, bold=True)
    date_para = doc.add_paragraph()
    _set_rtl_para(date_para)
    date_run = date_para.add_run(f"تاريخ التقرير: {datetime.now().strftime('%Y-%m-%d %H:%M')} | {VERSION}")
    _set_run_font(date_run, font_size=14, color=(100, 100, 100))
    doc.add_page_break()
    # الصور
    if images_b64:
    ...
    if image_analysis_text:
        ...
        if h.runs:
            h.runs[0].font.color.rgb = RGBColor(0, 120, 212)
            h.runs[0].font.size = Pt(20)
        for line in image_analysis_text.split('\n'):
            if not line.strip():
                doc.add_paragraph()
                continue
            if line.startswith('###'):
    ...
    if exec_summary:
        ...
        for line in exec_summary.split('\n'):
            ...
            if line.startswith('###'):
                ...
                _set_rtl_para(p)
                if p.runs:
                    p.runs[0].font.color.rgb = RGBColor(233,69,96)
    ...
    buf.seek(0)
    return buf.getvalue()
    def export_to_pptx(title, account_data=None, tweet_data=None, report_text="",
                   images_b64=None, image_analysis_text="", exec_summary=""):
    prs = Presentation()
    ...
    add_bg(slide)
    logo_box = slide.shapes.add_textbox(Emu(300000), Emu(400000), W-Emu(600000), Emu(800000))
    ...
    title_box = slide.shapes.add_textbox(Emu(300000), Emu(1500000), W-Emu(600000), Emu(900000))
    tf = title_box.text_frame
    tf.word_wrap = True
    p = tf.paragraphs[0]
    set_para_rtl_pptx(p)
    r = p.add_run()
    r.text = safe_text(title)
    r.font.size = PPt(28)
    r.font.bold = True
    r.font.color.rgb = WHITE
    date_box = slide.shapes.add_textbox(Emu(300000), Emu(3500000), W-Emu(600000), Emu(500000))
    tf = date_box.text_frame
    p = tf.paragraphs[0]
    set_para_rtl_pptx(p)
    r = p.add_run()
    r.text = f"{datetime.now().strftime('%Y-%m-%d')}  |  {VERSION}"
    r.font.size = PPt(16)
    ...
    if images_b64:
        for i, b64 in enumerate(images_b64[:3]):
            ...
            if bio:
                try:
                    slide.shapes.add_picture(bio, Emu(int(W*0.1)), Emu(1000000), Emu(int(W*0.8)), Emu(int(H*0.7)))
                except:
                    pass
    ...
    # ===== شرائح الملخص التنفيذي - جديد v10.2 =====
    if exec_summary:
        ...
        for ci, chunk_lines in enumerate(chunks):
            ...
            # خط فاصل أحمر
            try:
                line_box = slide.shapes.add_textbox(Emu(300000), Emu(1000000), W-Emu(600000), Emu(50000))
                ...
                pass
            add_content_box(slide, '\n'.join(chunk_lines), top=Emu(1100000), font_size=15)

    # شريحة ختامية
    slide = prs.slides.add_slide(prs.slide_layouts[6])
    add_bg(slide)
    end_box = slide.shapes.add_textbox(Emu(300000), Emu(2000000), W-Emu(600000), Emu(1000000))
    ...
    with col1:
        try:
            word_bytes = export_to_word(title=title, account_data=account_data,
                tweet_data=tweet_data, report_text=report_text, images_b64=images_b64,
                image_analysis_text=image_analysis_text, exec_summary=exec_summary)
            st.download_button("📄 تحميل Word", data=word_bytes,
    ...
    with col2:
        try:
            pptx_bytes = export_to_pptx(title=title, account_data=account_data,
                tweet_data=tweet_data, report_text=report_text, images_b64=images_b64,
                image_analysis_text=image_analysis_text, exec_summary=exec_summary)
            st.download_button("📊 تحميل PowerPoint", data=pptx_bytes,
    ...
    with col3:
        ...
        st.download_button("📝 تحميل TXT", data='\n'.join(parts).encode('utf-8'),
            file_name=f"{file_prefix}.txt", mime="text/plain", use_container_width=True)
...
def render_profile_card(data):
    ...
    img_src = f"data:image/jpeg;base64,{avatar_b64}" if avatar_b64 else "https://abs.twimg.com/sticky/default_profile_images/default_profile_400x400.png"
    ...
    st.markdown(f"""
    <div style="background:linear-gradient(135deg,#1a1a2e,#16213e,#0f3460);border:2px solid #0078d4;
                border-radius:16px;padding:20px;direction:rtl;text-align:right;font-family:'Segoe UI',Arial,sans-serif;margin-bottom:20px;">
        <div style="display:flex;align-items:center;gap:20px;flex-direction:row-reverse;">
            <img src="{img_src}" style="width:80px;height:80px;border-radius:50%;border:3px solid #0078d4;"/>
            <div>
        ...
        <div style="display:flex;justify-content:flex-end;gap:30px;margin-top:16px;flex-wrap:wrap;">
            <div style="text-align:center;"><div style="font-size:20px;font-weight:bold;color:#0078d4;">{followers}</div><div style="color:#aaa;font-size:13px;">متابع</div></div>
            <div style="text-align:center;"><div style="font-size:20px;font-weight:bold;color:#0078d4;">{following}</div><div style="color:#aaa;font-size:13px;">يتابع</div></div>
            <div style="text-align:center;"><div style="font-size:20px;font-weight:bold;color:#0078d4;">{tweets}</div><div style="color:#aaa;font-size:13px;">تغريدة</div></div>
        </div>
...
def display_tweet_card(data):
    ...
    st.markdown(f"""
    <div style="background:#16213e;border:1px solid #0078d4;border-radius:12px;padding:16px;
                direction:rtl;text-align:right;font-family:'Segoe UI',Arial,sans-serif;margin-bottom:16px;">
        ...
        <div style="display:flex;gap:20px;justify-content:flex-end;flex-wrap:wrap;color:#aaa;font-size:14px;">
            <span>❤️ {likes}</span><span>🔁 {retweets}</span>
            <span>💬 {replies}</span><span>👁️ {views}</span>
            <span>🌐 {lang}</span><span>📅 {created}</span>
        </div>
    </div>""", unsafe_allow_html=True)
    ...
    with col1:
        uploaded_profile = st.file_uploader("رفع صورة الملف الشخصي", type=["jpg","jpeg","png","webp"], key="account_img_upload")
    with col2:
        username_input = st.text_input("اسم المستخدم أو رابط الحساب", placeholder="@username", key="account_username")
    ...
    st.selectbox("نقطة تحليل الصورة", list(IMAGE_ANALYSIS_POINTS.keys()), key="account_analysis_point")
    if uploaded_profile and api_key:
        if st.button("🔍 تحليل صورة الملف الشخصي", key="btn_analyze_profile_img"):
            with st.spinner("جاري التحليل..."):
                point = st.session_state.get('account_analysis_point', list(IMAGE_ANALYSIS_POINTS.keys())[0])
    ...
    if st.session_state.get('account_img_analysis'):
        ...
        st.markdown(f'<div style="background:#0f3460;border-radius:10px;padding:16px;direction:rtl;text-align:right;color:#eee;">{img_html}</div>', unsafe_allow_html=True)
    if account_data and api_key:
        st.markdown("---")
        if st.button("📊 توليد تقرير تحليل الحساب", key="btn_account_report", use_container_width=True):
            prompt = f"""أنت محلل استخباراتي. حلّل حساب X التالي:
الاسم: {account_data.get('name','')}
...
يتابع: {format_number(account_data.get('following_count',0))}
التغريدات: {format_number(account_data.get('tweets_count',0))}
تحليل الصورة: {st.session_state.get('account_img_analysis','لا يوجد')}
...
"""
    ...
    if st.session_state.get('account_report'):
        ...
        st.markdown(f'<div style="background:#16213e;border-radius:10px;padding:20px;direction:rtl;text-align:right;color:#eee;line-height:1.8;">{r_html}</div>', unsafe_allow_html=True)
    ...
    tweet_input = st.text_input("رابط التغريدة أو معرّفها", placeholder="https://x.com/user/status/123...", key="tweet_input")
    ...
    st.markdown("---")
    uploaded_imgs = st.file_uploader("رفع صور للتحليل", type=["jpg","jpeg","png","webp"],
        accept_multiple_files=True, key="tweet_imgs_upload")
    all_images_b64 = []
    if tweet_data:
    ...
    if all_images_b64 and api_key:
        st.selectbox("اختر نقطة تحليل الصورة", list(IMAGE_ANALYSIS_POINTS.keys()), key="tweet_analysis_point")
        if st.button("🔍 تحليل الصور", key="btn_analyze_tweet_imgs", use_container_width=True):
            with st.spinner("جاري تحليل الصور..."):
                point = st.session_state.get('tweet_analysis_point', list(IMAGE_ANALYSIS_POINTS.keys())[0])
                prompt = IMAGE_ANALYSIS_POINTS[point]
    ...
    if st.session_state.get('tweet_img_analysis'):
        ...
        st.markdown(f'<div style="background:#0f3460;border-radius:10px;padding:16px;direction:rtl;text-align:right;color:#eee;line-height:1.8;">{img_html}</div>', unsafe_allow_html=True)
    # تحليل نص التغريدة
    if tweet_data and api_key:
        st.markdown("---")
        if st.button("📝 تحليل نص التغريدة", key="btn_analyze_tweet_text", use_container_width=True):
            prompt = f"""أنت محلل استخباراتي. حلّل التغريدة التالية:
النص: {tweet_data.get('text','')}
الكاتب: {tweet_data.get('author_name','')} (@{tweet_data.get('author_username','')})
التاريخ: {format_date(tweet_data.get('created_at',''))}
الإعجابات: {format_number(tweet_data.get('likes',0))}
إعادة التغريد: {format_number(tweet_data.get('retweets',0))}
المشاهدات: {format_number(tweet_data.get('views',0))}
المتابعون: {format_number(tweet_data.get('author_followers',0))}
تحليل الصورة: {st.session_state.get('tweet_img_analysis','لا توجد صور')}
...
"""
    ...
    if st.session_state.get('tweet_analysis'):
        ...
        st.markdown(f'<div style="background:#16213e;border-radius:10px;padding:20px;direction:rtl;text-align:right;color:#eee;line-height:1.8;">{a_html}</div>', unsafe_allow_html=True)

    # ===== الملخص التنفيذي - جديد v10.2 =====
    if tweet_data and api_key:
        st.markdown("---")
        st.markdown("""
        <div style="background:linear-gradient(135deg,#1a0a1e,#2d1b3d);border:2px solid #e94560;
                    border-radius:12px;padding:12px 20px;margin-bottom:10px;direction:rtl;text-align:right;">
            <h3 style="color:#e94560;margin:0;">📋 الملخص التنفيذي</h3>
            <p style="color:#ccc;margin:5px 0 0 0;font-size:14px;">تقرير موحّد يجمع جميع نتائج التحليل في ملخص احترافي للقيادة</p>
        </div>""", unsafe_allow_html=True)

        if st.button("🧠 توليد الملخص التنفيذي الشامل", key="btn_exec_summary", use_container_width=True):
            context_parts = []
            if tweet_data:
                context_parts.append(f"""بيانات التغريدة:
- الكاتب: {tweet_data.get('author_name','')} (@{tweet_data.get('author_username','')})
- النص: {tweet_data.get('text','')}
- التاريخ: {format_date(tweet_data.get('created_at',''))}
- الإعجابات: {format_number(tweet_data.get('likes',0))} | إعادة التغريد: {format_number(tweet_data.get('retweets',0))} | المشاهدات: {format_number(tweet_data.get('views',0))}""")
            if st.session_state.get('tweet_analysis'):
                context_parts.append(f"تحليل النص:\n{st.session_state['tweet_analysis'][:1500]}")
            if st.session_state.get('tweet_img_analysis'):
                context_parts.append(f"تحليل الصورة:\n{st.session_state['tweet_img_analysis'][:1000]}")

            sep = "\n---\n"
            exec_prompt = f"""أنت محلل استخباراتي كبير. بناءً على المعلومات التالية:
...
"""
    ...
    if st.session_state.get('exec_summary'):
        ...
        <div style="background:linear-gradient(135deg,#1a0a1e 0%,#16213e 50%,#0f3460 100%);
            ...
            <div style="display:flex;align-items:center;gap:10px;flex-direction:row-reverse;margin-bottom:16px;">
    ...
    # أزرار التصدير
    if (st.session_state.get('tweet_analysis') or st.session_state.get('tweet_img_analysis')
    ...
    with st.sidebar:
        ...
        api_key = st.text_input("🔑 مفتاح Gemini API", type="password",
            placeholder="أدخل مفتاح Gemini API...", key="gemini_api_key")
        ...
        st.markdown("---")
        model_choice = st.selectbox("🤖 نموذج Gemini", list(GEMINI_MODELS.keys()), key="gemini_model_choice")
        ...
        if debug:
            st.json({k: bool(st.session_state.get(k)) for k in
                ['account_data','tweet_data','account_report','tweet_analysis',
                 'account_img_analysis','tweet_img_analysis','exec_summary']})
...
def main():
    ...
    * { direction: rtl; }
    html, body, [class*="css"] { font-family: 'Cairo','Segoe UI',Arial,sans-serif !important; font-size: 16px; }
    ...
    .stButton > button:hover { background: linear-gradient(135deg, #005a9e, #003d6b); transform: translateY(-1px); }
    .stTextInput > div > div > input, .stTextArea textarea {
        background: #16213e; color: #eee; border: 1px solid #0078d4;
        border-radius: 8px; font-size: 16px; direction: rtl; text-align: right;
    }
    .stSelectbox > div > div { background: #16213e; color: #eee; direction: rtl; }
    .stDownloadButton > button { background: linear-gradient(135deg, #107c10, #0a5a0a); color: white; border: none; border-radius: 8px; }
    ...
    .stTabs [aria-selected="true"] { color: #0078d4 !important; border-bottom: 2px solid #0078d4; }
    div[data-testid="stSidebarContent"] { background: #0f0f2a; }
    </style>""", unsafe_allow_html=True)

    for key in ['account_data','tweet_data','account_report','tweet_analysis',
                'account_img_analysis','tweet_img_analysis','exec_summary','account_exec_summary']:
        if key not in st.session_state:
            st.session_state[key] = None

    api_key, model_name = render_sidebar()
    st.title("🔍 محلل حسابات X الاستخباراتي")
    st.markdown(f"<p style='color:#aaa;direction:rtl;text-align:right;'>الإصدار {VERSION} | مدعوم بـ Gemini AI</p>", unsafe_allow_html=True)
